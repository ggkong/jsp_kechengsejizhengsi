...
public class ImageServlet  extends HttpServlet{
    public void doGet(HttpServletRequest request,HttpServletResponse response) throws IOException{
        //这个方法实现验证码的生成
        BufferedImage bi=new BufferedImage(150, 68,BufferedImage.TYPE_INT_RGB);//创建图像缓冲区
        Graphics g=bi.getGraphics(); //通过缓冲区创建一个画布
        Color c=new Color(252, 252, 252); //创建颜色
        /*根据背景画了一个矩形框
         */
        g.setColor(c);//为画布创建背景颜色
        g.fillRect(0, 0, 150,68); //fillRect:填充指定的矩形
        ...
        for(int i=0;i<4;i++)
        {
            ...
            g.drawString(ch[index]+"",(i*30)+3, 60);//画数字以及数字的位置
            sb.append(ch[index]);
        }
